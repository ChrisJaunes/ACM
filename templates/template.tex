...
namespace BCC{
    ...
    int is_vertex[MAXN], vbcc_cnt, vbccno[MAXN];                   //vertex_cut
    vector<int> vbcc[MAXN];                                        //vertex_bcc
    stack<int> vS;                                                 //vertex_bcc

    int is_bridge[MAXM <<1], ebcc_cnt, ebccno[MAXN];               //bridge_bcc
    stack<int> eS;                                                 //bridge_bcc
    ...
    inline void init(int n) {
        ecnt = 1;
        dfs_clock = 0;
        vbcc_cnt = 0;                                             //vertex_bcc
        ebcc_cnt = 0;                                             //bridge_bcc
        for(int i = 1; i <= n; ++i){
            head[i] = dfn[i] = low[i] = 0;
            is_vertex[i] = 0;                                     //vertex_cut
            vbccno[i] = 0;                                        //vertex_bcc
            ebccno[i] = 0;                                        //bridge_bcc
        }
        while(!vS.empty()) vS.pop();                              //vertex_bcc
        for(int i = 1; i <= ecnt; ++i)is_bridge[i] = 0;           //bridge_cnt
    }
    //root's edge = -1;
    void tarjan(int u, int edge) {
        dfn[u] = low[u] = ++dfs_clock;
        int ch = 0;                                               //vertex_cut
        vS.push(u);                                               //vertex_bcc
        eS.push(u);                                               //bridge_bcc
        for(int i = head[u], v; i; i = e[i].nxt) {
            if(!dfn[v = e[i].to]) {
                tarjan(v, i ^ 1);
                low[u] = min(low[u], low[v]);
                if(low[v] >= dfn[u]) {                            //vertex_cut
                    ++ch;                                         //vertex_cut
                    if(edge > 0 || ch > 1) is_vertex[u] = 1;      //vertex_cut
                    vbcc[++vbcc_cnt].clear();                     //vertex_bcc
                    vbcc[vbcc_cnt].push_back(u);                  //vertex_bcc
                    for(int x;;){                                 //vertex_bcc
                        x = vS.top();vS.pop();                    //vertex_bcc
                        vbcc[vbcc_cnt].push_back(x);              //vertex_bcc
                        vbccno[x] = vbcc_cnt;                     //vertex_bcc
                        if(x == v)break;                          //vertex_bcc
                    }                                             //vertex_bcc
                }                                                 //vertex_cut
                if(low[v] > dfn[u]){                              //bridge_cut
                    is_bridge[i] = is_bridge[i ^ 1] = 1;          //bridge_cut        
                }                                                 //bridge_cut
            }
            else if(dfn[v] < dfn[u] && i != edge) 
                low[u] = min(low[u], dfn[v]);
        }
        if(dfn[u] == low[u]){                                     //bridge_bcc
            ebcc_cnt++;                                           //bridge_bcc
            for(int v;;){                                         //bridge_bcc
                v = eS.top(); eS.pop();                           //bridge_bcc
                ebccno[v] = ebcc_cnt;                             //bridge_bcc
                if(v == u) break;                                 //bridge_bcc
            }                                                     //bridge_bcc
        }                                                         //bridge_bcc
    }
    void findBCC(int n){
...
struct PalindromicTree {
    ...
    int nxt[MAXN][sigma_size]; //编号为i的节点表示的回文串在两边添加字符c以后变成的回文串的编号
    int S[MAXN]; //存放添加的字符(S[0]=-1(or '#'))
    int last, p, n;
    int newnode(int l) {
        for(int i = 0; i < sigma_size; i++) nxt[p][i] = 0;
        cnt[p] = 0;
        num[p] = 0;
        len[p] = l;
        return p++;
