...
namespace SAM{
	struct State{
		...
		void init(int _val, State *q) {
			fa = q->fa; val = _val;
			sc = 0;
			tmpl = 0; minl = INF;
			memcpy(ch, q->ch,sizeof ch);
		}
	}st[MAXN << 1];
	int scnt, root, last;
	void init() {
		st[root = last = scnt = 1].init(0);
	}
	void extend(int c) {
		int p = last, np = ++scnt;
		st[np].init(st[p].val + 1);
		for(;p && st[p].ch[c] == 0; p = st[p].fa) st[p].ch[c] = np;
		if(!p) st[np].fa = root;
		else{
			int q = st[p].ch[c];
			if(st[p].val + 1 == st[q].val) st[np].fa = q;
			else{
				int nq = ++scnt;
				st[nq].init(st[p].val + 1, &st[q]);
				st[np].fa = st[q].fa = nq;
	...
	void radix_sort(int n){
		...
		for(int i = 1; i <= scnt; i++) c[st[i].val]++;
		for(int i = 1; i <= n; i++) c[i] += c[i-1];
		for(int i = scnt; i >= 1; i--) rs[c[st[i].val]--] = i;
	}
	void go(){
		scanf("%d",s);
		int n = strlen(s);
		...
		for(; ~scanf("%s",s);) {
			...
			for(int i = 0, x; i < n; i++) {
				...
				if(st[cur].ch[x]) {
					...
					for(;cur && st[cur].ch[x] == 0; cur = st[cur].fa);
					if(cur){
						len = st[cur].val + 1;
						cur = st[cur].ch[x];
					}else{
				...
				st[cur].tmpl = max(st[cur].tmpl, len);
			}
			for(int i = scnt; i ; --i) {
				if( st[ st[rs[i]].fa ].tmpl < st[ rs[i] ].tmpl)
					st[ st[rs[i]].fa ].tmpl = st[ st[rs[i]].fa ].val;
				st[ rs[i] ].minl = min(st[ rs[i] ].minl, st[ rs[i] ].tmpl);
			}
		}
	}
}
