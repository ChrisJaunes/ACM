...
int ls[MAXN*30],rs[MAXN*30],fa[MAXN*30],dep[MAXN*30];
int Rt[MAXN*30];
namespace Persistant_Union_Set {
    ...
    void merge(int last,int &cur,int l,int r,int pos,int Fa) {
        cur=++tcnt; ls[cur]=ls[last]; rs[cur]=rs[last];
        if(l==r) {
            fa[cur]=Fa;
            dep[cur]=dep[last];
            return;
        ...
        int mid = (l+r) >> 1;
        if(pos<=mid)merge(ls[last],ls[cur],l, mid, pos,Fa);
        else merge(rs[last],rs[cur],mid+1, r, pos,Fa);
    }
    void update(int cur,int l,int r,int pos) {
        if(l==r){dep[cur]++;return ;}
        int mid = (l+r)>>1;
        if(pos<=mid)update(ls[cur],l,mid,pos); else update(rs[cur],mid+1,r,pos);
    }
    int query(int cur,int l,int r,int pos) {
        if(l==r) {return cur;}
        int mid = (l+r)>>1;
        if(pos<=mid)return query(ls[cur],l,mid,pos); 
        return query(rs[cur],mid+1,r,pos);
    }
    int find(int cur, int n, int pos) {
        int now = query(cur,1, n,pos);
        if(fa[now]==pos)return now;
        return find(cur,n, fa[now]);
    }
}
...
int main() {
    ...
    for(int i = 1; i <= m; i++) {
        ...
        if(opt==1) {
            ...
            Rt[i] = Rt[i-1];
            posx = find(Rt[i],n,x); posy=find(Rt[i],n,y);
            if(fa[posx]!=fa[posy]) {
                if(dep[posx]>dep[posy])swap(posx,posy);
                merge(Rt[i-1],Rt[i],1,n,fa[posx],fa[posy]);
...
inline int get(int x) { return (fa[x] == -1 ? 0 : lazy[fa[x]]) + V[x]; }
int main() {
    int n, m; cin >> n >> m;
    for(int i = 1, u, v; i < n; i++) {
        cin >> u >> v;
...
\end{lstlisting}
2. 给定一棵n个结点的有根树，结点从1开始编号，根结点为1号结点，每个结点有一个正整数权值vi。 设x号结点的子树内（包含x自身）的所有结点编号为c1,c2..ck\\
定义val(X)的价值为：$ val(X) = (v_{c_1} + d(c_1, x)) \oplus (v_{c_2} + d(c_2, x)) \oplus ... \oplus (v_{c_k} + d(c_k, x))$\\
其中d(x,y)表示树上x号结点与y号结点间唯一简单路径所包含的边数,d(x,x)=0。$ \oplus $表示异或运算。 请你求出$ \sum_{i=1}^{n} {val[i]}$的结果\\
解：考虑每个结点对其所有祖先的贡献。 每个结点建立 trie，初始先只存这个结点的权值，然后从底向上合并每个儿子结点上的 trie，然后再全局加一，完成后统计答案
\begin{lstlisting}
#include <bits/stdc++.h>
using namespace std;
...
void solve() {
	...
	for(int i = scnt; i; i--) {
		for(int j = 0; j < 26; j++) 
			if(ch[rs[i]][j] ) g[rs[i]] += g[ch[ rs[i]][j] ];
	}
	if(g[1] < k) cout << -1 << endl;
	else{
		cur = root;
		while(k > f[cur]) {
			k -= f[cur];
...
\end{lstlisting}
6. 所有字符中不同子串个数，真广义自动机\\
可以根据后缀自动机的性质得到，以点i为结束节点的子串个数等于len[i] - len[link[i]]\\
所以可以遍历所有的节点求和得到
\begin{lstlisting}
#include<bits/stdc++.h>
using namespace std;
...
struct exSAM {
    ...
    int insertSAM(int last, int c) {
        ...
        for (int i = 0; i < CHAR_NUM; ++i)
            ch[clone][i] = len[ch[q][i]] != 0 ? ch[q][i] : 0;
        len[clone] = len[p] + 1;
        while (p != -1 && ch[p][c] == q) {
            ch[p][c] = clone;
            p = fa[p];
    ...
    int insertTrie(int cur, int c) {
        if (ch[cur][c]) return ch[cur][c];
        return ch[cur][c] = scnt++;
    }
    void insert(const string &s) {
        int root = 0;
        for (auto it : s) root = insertTrie(root, it - 'a');
    }
    ...
    void build() {
        ...
        for (int i = 0; i < 26; ++i) if (ch[0][i]) q.push({i, 0});
        while (!q.empty()) {
            auto item = q.front();
            q.pop();
...
namespace SA{
    ...
    void buildHeight() {
        ...
        //for(int i = 0; i <= n; i++) cerr <<">" << i << " " << rnk[i] << endl;
        //for(int i = 0; i <= n; i++) cerr <<"!" << i << " " << sa[i] << endl;
    }
    ...
    pair<int, int> getLR(int len, int pos) {
        pair<int, int> LR;
        int l = 1, r = pos, mid;
        while(l < r) {
            mid = (l+r)>>1;
            if(lcp(mid+1, pos) >= len) r = mid; else l=mid+1;
        }
        LR.first = l;
        l = pos, r = n;
        while(l < r) {
            mid=(l+r+1)>>1;
            if(lcp(pos+1,mid) >= len) l=mid; else r=mid-1;
        }
        LR.second = l;
        return LR;
    }
}
struct Node {int ls,rs,sum;} tr[MAXN*40];
int rt[MAXN], tcnt;
void insert(int &cur, const int &las, const int &l, const int &r, const int &pos) {
    tr[cur = ++tcnt] = tr[las]; ++tr[cur].sum;
    if(l==r) return;
    int mid = (l + r) >> 1;
    if(pos <= mid) insert(tr[cur].ls, tr[las].ls, l, mid, pos);
    else insert(tr[cur].rs, tr[las].rs, mid+1, r, pos);
}
int query(int x, int y, int l, int r, int ql, int qr) {
    if(ql <= l && r <= qr) return tr[y].sum-tr[x].sum;
    int mid=(l+r)>>1;
    int res=0;
    if(ql<=mid) res+=query(tr[x].ls, tr[y].ls, l, mid, ql, qr);
    if(qr>mid)  res+=query(tr[x].rs, tr[y].rs, mid+1,r,ql, qr);
    return res;
}
int main() {
    int n, m; 
    scanf("%d%d%s", &n,&m,SA::s);
    SA::init();
    for(int i=1;i<=n;++i) insert(rt[i], rt[i-1], 0, n, SA::sa[i]);
    while(m--) {
        int a, b, c, d;
        ...
        while(l<=r) {
            int mid=(l+r)/2;
            auto LR = SA::getLR(mid, SA::rnk[c]);
            if (query(rt[LR.first-1], rt[LR.second],0,n,a,b-mid+1)) 
...
int c[MAXN], rnk[MAXN << 1];
void radix_sort(int n){
    for(int i = 0; i <= n; i++) c[i] = 0; 
    for(int i = 1; i <= scnt; i++) c[len[i]]++;
    for(int i = 1; i <= n; i++) c[i] += c[i-1];
    for(int i = scnt; i >= 1; i--) rnk[c[len[i]]--] = i;
}
char str[MAXN];
int ls[MAXN<<6],rs[MAXN<<6],sum[MAXN<<6],rt[MAXN<<1],tcnt;
void pushup(const int &rt) {sum[rt]=sum[ls[rt]]+sum[rs[rt]];}
...
void update(int &rt,const int &l,const int &r,const int &p) {
    if(!rt) rt=++tcnt;
    if(l==r){sum[rt]++;return;}
    int mid=(l+r)>>1;
    if(p<=mid)update(ls[rt],l,mid,p); else update(rs[rt],mid+1,r,p);
    pushup(rt);
}
...
int F[MAXN<<1][21],pos[MAXN];
inline bool check(const int &lim,int p,const int &l,const int &r, const int &n) {
    for(int i=20;~i;i--)if(len[F[p][i]]>=lim&&F[p][i]) p=F[p][i];
...
int main() {
    ...
    init();
    for(int i=1;i<=n;i++) {extend(str[i]-'a');update(rt[last],1,n,i);pos[i]=last;}
    radix_sort(n);
    for(int i=scnt;i>=1;i--)if(fa[rnk[i]]){
        rt[fa[rnk[i]]]=merge(rt[fa[rnk[i]]],rt[rnk[i]],1,n);
        F[i][0]=fa[i];
    }
    for(int j=1;j<=20;j++)
        for(int i=1;i<=scnt;i++)
    ...
    while(m--) {
        ...
        int l=1,r=min(a-b+1,c-d+1);
        while(l<=r) {
            int mid=(l+r)>>1;
            if(check(mid,pos[c],b,a,n))l=mid+1;else r=mid-1;
        }
        printf("%d\n", l-1);
    }
    return 0;
...
分治FFT:\\
解决一类自己更新自己的卷积问题。\\
给定序列$g_{1...n-1}$，求序列$f_{0...n-1}$，其中$f_i = \sum_{j=1}^i f_{i-j}g_j$, 边界$f_0 = 1$
\begin{lstlisting}
#include <bits/stdc++.h>
using namespace std;
typedef long long LL;
...
Example: 2020CCPC(net) 1013
$$
f_1(x) = \sum_{i=0}^n a_i x^i \\
f_i(x) = b_i(f_{i-1}(x))' + c_i f_{i-1}(x)
$$
Calculate $f_n(x)$ mod $998244353$ \\

Solution:
令$f_i(x) = \sum_{j=0}^n d_{i,j}f_1(x)^{(j)}$，是$f_1(x)$若干阶导的线性组合\\
令$g_i(x) = \sum_{j=0}^n d_{i,j}x^{j}$\\
由$g_i(x) = b_i(g_{i-1}(x))x + c_i g_{i-1}(x)$，可得
$$
g_n(x) = \prod_{i=2}^n (b_ix + c_i)
$$
分治FFT求$g_n$ \\
考虑如何求$[x^i]f_n(x)$，即$f_n(x)$中$x_i$的系数。\\
考虑每一阶导对它的贡献，对于$[x^j]f_1(x)$，一定是求了$j-i$次导以后产生贡献，则
$$
[x^i]f_n(x) = \sum_{j=i}^n d_{n,j-i} (\prod_{k=i+1}^j k) ([x^j]f_1(x))
$$
即
$$
[x^i]f_n(x) = \frac{1}{i !} \sum_{j=i}^n d_{n, j-i} (j! a_j)
$$
翻转下标，令$A_{n-j} = j!a_j$，有
$$
[x^{n-i}]f_n(x) = \frac{1}{i!} \sum_{j=i}^n d_{n, j-i} A_{n-j}
$$
FFT加速即可
...
using namespace std;
typedef long long LL;
const int MAXN = 5e5 + 5, G = 3, mod = 998244353;
namespace NTT {
    int rev[MAXN], T;
    LL qpow(LL x, LL y) {
        LL res = 1;
...
LL f[MAXN], g[MAXN], A[MAXN], B[MAXN], C[MAXN];
LL x[MAXN], y[MAXN];
void solve(int l, int r) {
    if(l == r) {
        f[0] = C[l + 2];
    ...
    solve(mid + 1, r);
    NTT::init(r - l + 1);
    for(int i = 0; i < NTT::T; i++) x[i] = y[i] = 0;
    for(int i = 0; i <= mid - l + 1; i++) x[i] = tmp[i];
    for(int i = 0; i <= r - mid; i++) y[i] = f[i];
    NTT::cal(x, y);
    for(int i = 0; i <= r - l + 1; i++) {
...
\newpage
\section{Geometry}
\subsection{The Number of Ingeter Point on a Circle}
...
\begin{lstlisting}
    ...
    dp[i][j] 表示队列中当前有i个人，且站在第j个位置上时最终会到达终态的概率。
    如果1<j<=k，那么dp[i][j]=p1∗dp[i][j]+p2∗dp[i][j−1]+p3∗dp[i−1][j−1]+p4;考虑当前状态到达终态由四种转移而来：激活失败、失去连接、服务器瘫痪。
    这里j==1特判一下，有dp[i][1]=dp[i][1]∗p1+p2∗dp[i][i]+p4;
    如果k<j<=i，那么dp[i][j]=p1∗dp[i][j]+p2∗dp[i][j−1]+p3∗dp[i−1][j−1]；那么当前状态不可能通过服务器瘫痪到达终态。

    令tp2=p2/(1−p1),tp3=p3/(1−p1),tp4=p4/(1−p1)。已知量用v[j]表示。

    dp[i][1]=dp[i][i]∗tp2+v[1].....j=1
    dp[i][j]=dp[i][j−1]∗tp2+v[j].....2<=j<=i

    (1.0−(tp2)^i)dp[i][i]=∑^i_(j=1)c[j]∗(tp2)^(i−j)。
*/
...
\begin{lstlisting}
    /*
    题意：DAG，起点为1且仅有出度，终点为n且仅有入读，机器人等概率走到下个点或停留原地，每日耐久消耗比前一天多1
    双重期望，先求出每个点到终点的期望天数E[i]，这里期望可以看每天耐久消耗恒为1，再求每个点到终点的期望耐久消耗F[i]，此时每个点的耐久消耗就是E[i]
*/
